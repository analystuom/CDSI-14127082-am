...
const NegativeReviewsScatter = ({ scatterData, selectedProductIds, isLoading, error }) => {
  ...
  // Create scatter plot data for negative reviews vs total reviews
  const createNegativeScatterPlotData = (scatterData, productIds) => {
    ...
    const scatterPlotData = productIds.map((productId, index) => {
      ...
      return {
        ...
        data: [{
          ...
          y: productStats.total_negative_reviews,
          productId: productId,
          totalReviews: productStats.total_reviews,
          negativeReviews: productStats.total_negative_reviews,
  ...
  // Chart options for negative reviews scatter plot
  const getNegativeScatterPlotOptions = () => ({
    responsive: true,
        ...
        callbacks: {
          ...
          label: function(context) {
            ...
            return [
              ...
              `Negative Reviews: ${dataPoint.negativeReviews.toLocaleString()}`
            ];
          }
        }
      }
    ...
    scales: {
      ...
      y: {
        ...
        title: {
          display: true,
          text: 'Total Number of Negative Reviews',
          font: {
            size: 14,
            weight: 'bold'
          }
  ...
  if (isLoading) {
    return (
      <div className="bg-gray-200 border border-gray-400 rounded-xl p-6 shadow-lg">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Negative Reviews vs Total Reviews</h3>
  ...
  // Create chart data
  const negativeScatterData = createNegativeScatterPlotData(scatterData, selectedProductIds);

  if (!negativeScatterData) {
    return (
      <div className="bg-gray-200 border border-gray-400 rounded-xl p-6 shadow-lg">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Negative Reviews vs Total Reviews</h3>
        <div className="text-center py-8">
  ...
  return (
    <div className="bg-gray-200 border border-gray-400 rounded-xl p-6 shadow-lg">
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-2">Negative Reviews vs Total Reviews</h3>
        <p className="text-sm text-gray-600">
          Each point represents a product plotted by total reviews and total negative reviews
        </p>
      </div>
      ...
      <div className="h-96">
        <Scatter 
          ...
          options={getNegativeScatterPlotOptions()}
        />
      </div>

      {/* Product Performance Summary
      <div className="mt-6 pt-6 border-t border-gray-300">
        <h4 className="font-semibold text-gray-800 mb-3">Product Performance Summary - Negative Reviews</h4>
                ...
                <div className="space-y-1 text-xs text-gray-700">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-red-600">
                      {productStats.total_negative_reviews.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Negative Rate:</span>
                    <span className="font-medium text-red-600">
                      {productStats.total_reviews > 0 ? ((productStats.total_negative_reviews / productStats.total_reviews) * 100).toFixed(1) : 0}%
                    </span>
                  </div>
                </div>
              </div>
...
export default NegativeReviewsScatter;
